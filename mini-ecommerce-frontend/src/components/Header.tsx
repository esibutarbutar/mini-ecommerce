...
const Header: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, position: 'relative', flex: 1, justifyContent: 'flex-end' }}>
        {isLoggedIn && (
          <button
            onClick={() => navigate('/history')}
            style={{ background: '#FF7043', color: '#fff', border: 'none', borderRadius: 8, padding: '0.5rem 1.2rem', fontWeight: 'bold', fontSize: '1rem', cursor: 'pointer', marginRight: 8 }}
          >
            History
          </button>
        )}
        <span
          ...
          title="Profil"
        >
          <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor" style={{ display: 'block' }}><circle cx="12" cy="8" r="4"/><path d="M4 20c0-2.5 4-4 8-4s8 1.5 8 4"/></svg>
        </span>
      </div>
    </header>
  );
